...
The Virtual Private Network (VPN) enable staff to securely connect to the Water Board's network and access various online resources including the Intranet (waternet) and shared drives. VPN is used on Water Boards' approved devices to access the network when working remotely. See the [VPN User Instructions](http://waternet.waterboards.ca.gov/dit/help_desk/docs/vpn_user_instructions.pdf).

All staff are required to use Multi-Factor Authentication (MFA) when accessing the Water Boards' network. See the [Remote Access Guidence Page](https://waternet.waterboards.ca.gov/dit/help_desk/#remote) for instructions to set up your MFA method.

Your supervisor and DIT can help you get set up.

## Installing Printer(s)

[Instructions on how to install printer(s)](http://printermap/PrinterMap/) near your cubicle in the office.
